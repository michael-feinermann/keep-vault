...
internal static class ReleaseEndToEndPerformanceTests
{
    ...
    private static async Task CreateComplexTreeFixtureAsync(string root)
    {
        ...
        [
            ...
            ("ähnliche Datei.txt", 127, DataPattern.Counter, 101),
            ("ähnliche Datei (1).txt", 128, DataPattern.Counter, 102),
            ("ähnliche Datei_1.txt", 129, DataPattern.Counter, 103),
        ...
        ulong seed)
    {
        ...
        File.SetLastWriteTimeUtc(path, new DateTime(2026, 9, 1, 12, 34, 56, DateTimeKind.Utc));
    }
                     ...
                     .Order(StringComparer.Ordinal))
        {
            ...
            try
            {
                files.Add(new FileManifest(
                    ...
                    info.Length,
                    Convert.ToHexString(hash), info.LastWriteTimeUtc.Ticks / TimeSpan.TicksPerSecond, info.Attributes & (FileAttributes.ReadOnly | FileAttributes.Hidden | FileAttributes.System)));
    ...
    private static void RequireEquivalent(TreeManifest expected, TreeManifest actual)
    {
        ...
        Require(
            ...
            "The extracted path, size, SHA-256, timestamp or Windows attributes differ from the source tree.");
    }
    ...
    private sealed record FileManifest(string RelativePath, long Length, string Sha256, long LastWriteUtcSeconds, FileAttributes Attributes);
